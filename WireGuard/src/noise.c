...
void print_hex(const u8 *key, int key_len) {
...
static void handshake_zero(struct noise_handshake *handshake)
{
	...
	memset(&handshake->chaining_key, 0, NOISE_HASH_LEN);
        ...
        memset(&handshake->eph_sk, 0, NOISE_EPHEMERAL_SECRET_KEY_LEN);
        memset(&handshake->eph_pk, 0, NOISE_EPHEMERAL_PUBLIC_KEY_LEN);
...
{
	//print_sth(key, NOISE_SYMMETRIC_KEY_LEN, "wireguard key in enc ");
	chacha20poly1305_encrypt(dst_ciphertext, src_plaintext, src_len, hash,
				 NOISE_HASH_LEN,
				 0 /* Always zero for Noise_IK */, key);
	mix_hash(hash, dst_ciphertext, noise_encrypted_len(src_len));
	//print_sth(hash, NOISE_HASH_LEN, "wireguard hash in enc ");
}
...
{
	// test();
	pr_info("NOISE_EPHEMERAL_PUBLIC_KEY_LEN = %d", NOISE_EPHEMERAL_PUBLIC_KEY_LEN);
	pr_info("NOISE_EPHEMERAL_SECRET_KEY_LEN = %d", NOISE_EPHEMERAL_SECRET_KEY_LEN);
	pr_info("NOISE_EPHEMERAL_CIPHERTEXT_LEN = %d", SABER_BYTES_CCA_DEC);
	...
	memcpy(handshake->hash, handshake->precomputed_hash, NOISE_HASH_LEN);
		...
		crypto_kem_keypair(dst->ephemeral_public, handshake->eph_sk);
	...
	// dst->mc_ciphertext 是将要被传输的密文
	// key	是一个指针，指向存储共享秘密的空间，可以理解为存储了共享秘密
	// handshake->remote_mc_pk 是输入的pk
	// 据说是会自动截断的, key size = 32B
		if (crypto_kem_enc(dst->mc_ciphertext, key, handshake->remote_mc_pk)) {
			goto out;
		}
		//print_sth(dst->mc_ciphertext, KYBER_CIPHERTEXTBYTES, "wireguard ciphertext in create: ");
		//print_sth(key, NOISE_SYMMETRIC_KEY_LEN, "wireguard 1  ");

        // mix the fresh secret into chaining key, and generate key k1      KDF
        mix_s_gen(handshake->chaining_key, key, key, handshake->chaining_key);
		//print_sth(key, NOISE_SYMMETRIC_KEY_LEN, "wireguard 2  ");

        // encrypt the hash of our own static McEliece pk with k1, and mix
        // the ciphertext into hash
        message_encrypt(dst->encrypted_static,								// ltk
                        handshake->static_identity->static_hash,			// H(spki)
                        NOISE_PK_HASH_LEN, key, handshake->hash);			// k3, H3
		//print_sth(key, NOISE_SYMMETRIC_KEY_LEN, "wireguard 3  ");
		//print_sth(dst->encrypted_static, noise_encrypted_len(NOISE_PK_HASH_LEN), "wireguard encrypted static: ");
		//print_sth(key, NOISE_SYMMETRIC_KEY_LEN, "wireguard k3? ");


        // mix PSK into chaining key and generate key k2
        mix_s_gen(handshake->chaining_key, key, handshake->preshared_key,
                  handshake->chaining_key);
		//print_sth(key, NOISE_SYMMETRIC_KEY_LEN, "wireguard 4  ");

        // encrypt the timestamp with k2, and mix the ciphertext into hash
	tai64n_now(timestamp);
	message_encrypt(dst->encrypted_timestamp, timestamp,
                        NOISE_TIMESTAMP_LEN, key, handshake->hash);
		//print_sth(key, NOISE_SYMMETRIC_KEY_LEN, "wireguard 5  ");
...
{	
	...
	peer = wg_pubkey_hashtable_random_free(wg->peer_hashtable);
		...
		//print_sth(src->mc_ciphertext, KYBER_CIPHERTEXTBYTES, "wireguard ciphertext in consume: ");
		//print_sth(wg->static_identity.static_mc_sk, KYBER_SECRETKEYBYTES, "wireguard sk(sk1 in wg1): ");
		// 错误就在这里！
		if (crypto_kem_dec(key, src->mc_ciphertext, wg->static_identity.static_mc_sk)) {
			up_write(&peer->handshake.lock);
            goto out;
		}

		// 可以尝试输出一下key？
		//print_sth(key, NOISE_SYMMETRIC_KEY_LEN, "wireguard 6  ");		// 这里应该和1一样, 但不一样
		
		// 这个输出的时候，带了一堆00000......原因是这里用了kyber768, 于是改成了kyber768
		// print_sth(wg->static_identity.static_mc_sk, KYBER_SECRETKEYBYTES, "wireguard static mc sk: ");	// 正确的
		// print_sth(src->mc_ciphertext, KYBER_CIPHERTEXTBYTES, "wireguard ciphertext rcvd: ");	//正确的
		...
		//print_sth(key, KYBER_SSBYTES, "wireguard k1?: ");
		// print_sth(key, NOISE_SYMMETRIC_KEY_LEN, "wireguard 7  ");

	// decrypt the hash of peer's static McEliece pk with k1, and mix
        // the ciphertext into hash
	if(!message_decrypt(static_hash, src->encrypted_static,			// 似乎是这里出问题了
	                    sizeof(src->encrypted_static), key, hash)) {
		pr_info("sgniws: something wrong in the decryption of static pk, so goto out\n");
		goto out;
        }
	pr_info("sgniws: noise.c, successfully decrypt message\n");
	// print_sth(key, NOISE_SYMMETRIC_KEY_LEN, "wireguard 8  ");
	...
	pr_info("sgniws: noise.c, successfully decrypt timestamp\n");
					...
					struct noise_handshake *handshake)
{
	...
	dst->receiver_index = handshake->remote_index;
		...
		pr_info("<noise.c: wg_noise_handshake_create_response>\n");
		if (crypto_kem_enc(dst->mc_ciphertext, key, handshake->remote_mc_pk)) {
			goto out;
		}
        ...
        /*saber_indcpa_enc(dst->ephemeral_ciphertext, key, handshake->eph_pk, coin,
                         &handshake->eph_buf);*/
		crypto_kem_enc(dst->ephemeral_ciphertext, key, handshake->eph_pk);
				    ...
				    struct wg_device *wg)
{
	...
	if (state != HANDSHAKE_CREATED_INITIATION) {
		...
		pr_info("<noise.c: wg_noise_handshake_consume_response>\n");
		if (crypto_kem_dec(key, src->mc_ciphertext, handshake->static_identity->static_mc_sk)) {
			up_write(&handshake->lock);
			pr_info("in wg noise handshake consume response goto fail\n");
            goto fail;
		}
		...
		crypto_kem_dec(key, src->ephemeral_ciphertext, handshake->eph_sk);
